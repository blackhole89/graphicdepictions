...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        else
        {
            ...
            // all windows have borders [blackhole89]
            window->BorderSize = 1.0f;//(flags & ImGuiWindowFlags_ShowBorders) ? 1.0f : 0.0f;
            ...
            // tweak to match old GUI style [blackhole89]
            if (bg_color.w > 0.0f)
            ...
            if (!(flags & ImGuiWindowFlags_NoTitleBar))
                window->DrawList->AddRectFilled(title_bar_rect.GetTL()+ImVec2(3,3), title_bar_rect.GetBR()+ImVec2(-3,0), GetColorU32((g.FocusedWindow && window->RootNonPopupWindow == g.FocusedWindow->RootNonPopupWindow) ? ImGuiCol_TitleBgActive : ImGuiCol_TitleBg), window_rounding, 1|2);
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        else if (!(flags & ImGuiInputTextFlags_CallbackEscape) && IsKeyPressedMap(ImGuiKey_Escape))                           { SetActiveID(0); cancel_edit = true; }
        ...
        else
        {
            ...
            // User callback
            if ((flags & (ImGuiInputTextFlags_CallbackCompletion | ImGuiInputTextFlags_CallbackHistory | ImGuiInputTextFlags_CallbackAlways | ImGuiInputTextFlags_CallbackEscape)) != 0)
            {
