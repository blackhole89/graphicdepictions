    ...
    else if (pass == 1)
    {
        ...
        for (int y = 0, n = 0; y < TEX_DATA_H; y++)
            for (int x = 0; x < TEX_DATA_W; x++, n++)
            {
                ...
                // [blackhole89]
                if(TexPixelsRGBA32) {
                    TexPixelsRGBA32[offset0] = texture_data[n] == '.' ? 0xFFFFFFFF : 0x00;
                    TexPixelsRGBA32[offset1] = texture_data[n] == 'X' ? 0xFFFFFFFF : 0x00;
                }
            }
...
void ImFont::RenderText(ImDrawList* draw_list, float size, ImVec2 pos, ImU32 col, const ImVec4& clip_rect, const char* text_begin, const char* text_end, float wrap_width, bool cpu_fine_clip) const
{
    ...
    const int vtx_count_max = (int)(text_end - s) * 8;
    const int idx_count_max = (int)(text_end - s) * 12;
            ...
            if (c != ' ' && c != '\t')
            {
                ...
                if (x1 <= clip_rect.z && x2 >= clip_rect.x)
                {
                    ...
                    x1+=2; y1+=2; x2+=2; y2+=2;
                    {
                        ...
                        vtx_write[0].pos.x = x1; vtx_write[0].pos.y = y1; vtx_write[0].col = 0xFF000000; vtx_write[0].uv.x = u1; vtx_write[0].uv.y = v1;
                        vtx_write[1].pos.x = x2; vtx_write[1].pos.y = y1; vtx_write[1].col = 0xFF000000; vtx_write[1].uv.x = u2; vtx_write[1].uv.y = v1;
                        vtx_write[2].pos.x = x2; vtx_write[2].pos.y = y2; vtx_write[2].col = 0xFF000000; vtx_write[2].uv.x = u2; vtx_write[2].uv.y = v2;
                        vtx_write[3].pos.x = x1; vtx_write[3].pos.y = y2; vtx_write[3].col = 0xFF000000; vtx_write[3].uv.x = u1; vtx_write[3].uv.y = v2;
