...
namespace {
	...
	void FreeTypeFont::SetPixelHeight( uint32_t pixelHeight ) {
		...
		// [blackhole89]
		req.width = 0; //(pixelHeight+1.0f)*64;
		req.height = pixelHeight * 64 +13;
...
bool ImGuiFreeType::BuildFontAtlas( ImFontAtlas* atlas, unsigned int flags ) {
	...
	atlas->TexPixelsRGBA32 = ( unsigned int* )ImGui::MemAlloc( atlas->TexWidth * atlas->TexHeight * sizeof(unsigned int) );
	memset( atlas->TexPixelsRGBA32, 0, atlas->TexWidth * atlas->TexHeight*sizeof(unsigned int) );
	...
	for( int input_i = 0; input_i < atlas->ConfigData.Size; input_i++ ) {
		...
		for( const ImWchar* in_range = cfg.GlyphRanges; in_range[ 0 ] && in_range[ 1 ]; in_range += 2 ) {
			for( uint32_t codepoint = in_range[ 0 ]; codepoint <= in_range[ 1 ]; ++codepoint ) {
				...
				uint32_t* dst = atlas->TexPixelsRGBA32 + rect.y * atlas->TexWidth + rect.x;
				uint8_t* dst8 = atlas->TexPixelsAlpha8 + rect.y * atlas->TexWidth + rect.x;
				for( uint32_t yy = 0; yy < glyphBitmap.height; ++yy ) {
                    ...
                    int dy = -(yy+glyphInfo.offsetY); //atlas->ConfigData.Size-glyphBitmap.height;
                    int vs = cfg.SizePixels*0.5;

					int factor = (dy>(2.3*vs/3)||dy<(0.5*vs/3))?0xB0:0xFF;
					for( uint32_t xx = 0; xx < glyphBitmap.width; ++xx) {
                        dst[xx] = (src[xx]<<24)|0xFF0000|factor<<8|factor;
                        dst8[xx] = src[xx];
                    }
				...
				glyph.Y0 = glyphInfo.offsetY-2; //raise all glyphs by 2px [blackhole89]
